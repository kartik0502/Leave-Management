...
def index(request):
    if request.session.get("username"):
        ...
        if reply:
            ...
            # if reply:
            context = {"user": reply, "nol_json": nol_json}
        ...
        if reply:
            ...
            context = {
                "user": reply,
                "leaves": leavetable.find(
                    {"email": request.session.get("username")}
                ).sort("from_date", pymongo.DESCENDING),
                "nol_json": nol_json,
            }
...
def pending_to_approved(request, oid):
    if request.session.get("username"):
        ...
        # iterate over each day in the date range and subtract weekends
        ...
        if document:
            ...
            studenttable.update_one({"email": email}, {"$set": {"nol": leaves}})
        ...
        if document:
            leaves = document["nol"]
            leaves = int(leaves) + (delta.days + 1)
            facultytable.update_one({"email": email}, {"$set": {"nol": leaves}})
        ...
        if document:
            leaves = document["nol"]
            leaves = int(leaves) + (delta.days + 1)
            tatable.update_one({"email": email}, {"$set": {"nol": leaves}})
        ...
        status = "approved"
        leaveTable.update_one(
            {"_id": id},
            {"$set": {"status": status}},  # update status of leave in leave table
        )
        ...
        send_mail(
            substring,
            bodystring_to_leave_applier,
            "leavemanagement91@gmail.com",
            own_email_list,
        )
        del emaillist[0]
        send_mail(
            substring, bodystring_to_emaillist, "leavemanagement91@gmail.com", emaillist
        )
...
def pending_to_rejected(request, oid):
    if request.session.get("username"):
        ...
        status = "rejected"
        leaveTable.update_one({"_id": id}, {"$set": {"status": status}})
    ...
    if request.method == "GET":
        ...
        if(data_type == None):
            ...
            if reply:  # if the user is a faculty
                faculty_data = facultytable.find_one({"email": request.session.get("username")})
            ...
            reply = tatable.find_one({"email": request.session.get("username")})

            if reply:  # if the user is a ta
                ta_data = tatable.find_one({"email": request.session.get("username")})
                context = {
                    "ta_data":ta_data,
                }
            ...
            if reply:
                leave_data = leavetable.find()
                data = []
                for leaves in leave_data:
                    if(leaves["role"] == data_type):
                        if(data_type == "student"):
                            student_info = studenttable.find_one({"email" : leaves["email"]})
                            for courses in student_info["courses"]:
                                if(courses == course):
                                    data.append(leaves)
                        
                        else:
                            ta_info = tatable.find_one({"email" : leaves["email"]})
                            for courses in ta_info["courses"]:
                                if(courses == course):
                                    data.append(leaves)
        ...
        if request.method == "POST":
            ...
            if reply:
                if check_password(old_password, reply["password"]) == False:
                    ...
                    facultytable.update_one(
                        {"email": reply["email"]}, {"$set": {"password": new_password}}
                    )
...
def change_password(request):
    if request.method == "POST":
        ...
        admin = db["adminTable"]
        reply = admin.find_one({"email": email})

        if reply:
            password = make_password(password)
            admin.update_one(
                {"email": reply["email"]}, {"$set": {"password": password}}
            )
            messages.success(request, "Password changed successfully!")
            # return render(request, 'faculty-profile.html',{'user':reply})
            return redirect("/admin_page")
